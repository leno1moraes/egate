...
$emailnotice = (isset($_GET['emailnotice'])) ? $_GET['emailnotice'] : "";

if (isset($_GET['emailnotice']) && $emailnotice != "" ) {
    $mail = new PHPMailer;
    ...
    $mail->setFrom('noreplay.egate@gmail.com', 'e-Gate Informa');

    //Set an alternative reply-to address
    //$mail->addReplyTo('replyto@example.com', 'First Last');

    //Set who the message is to be sent to
    $mail->addAddress($emailnotice, 'Responsável');

    //Set the subject line
    $mail->Subject = 'e-Gate IFMT Campus Avançado VRL Informa';

    //Read an HTML message body from an external file, convert referenced images to embedded,
    //convert HTML into a basic plain-text alternative body
    //$mail->msgHTML(file_get_contents('contents.php'), dirname(__FILE__));

    $dataReg = date('d/m/Y H:i:s', strtotime($dataReg));
    ...
    echo "E-mail not sent. Notice e-mail is empty";
}
